...
reconnect() {
    ...
    psql -e -v ON_ERROR_STOP=1 --username postgres --dbname testdb -c "SELECT pglogical.create_subscription(subscription_name := '$1',provider_dsn := 'host=192.168.99.149 port=5432 dbname=testdb password=pass user=postgres');"
