...
docker exec $1 psql -e -v ON_ERROR_STOP=1 --username primaryuser --dbname testdb -c "SELECT pglogical.create_subscription(subscription_name := '$2',provider_dsn := 'host=192.168.99.149 port=5433 dbname=testdb password=pass user=primaryuser');"
